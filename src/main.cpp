...
int main()
{
    ...
    Shader lightShader("../src/shader/lightShader/lightShader.vs", "../src/shader/lightShader/lightShader.fs");
    Shader lightCubeShader("../src/shader/lightShader/lightCubeShader.vs", "../src/shader/lightShader/lightCubeShader.fs");
    ...
    unsigned char* data0 = stbi_load("../res/container2.png", &width, &height, &nrChannels, STBI_rgb_alpha);
    ...
    if(data0)
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data0);
    ...
    data0 = stbi_load("../res/container2_specular.png", &width, &height, &nrChannels, STBI_rgb_alpha);
    ...
    // data0 = stbi_load("../res/matrix.jpg", &width, &height, &nrChannels, 0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // clear buffer and color
        glClearColor(0.1f, 0.1f, 0.1f, 1.0f);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

        // activate shader
        lightShader.use();
        glm::vec3 lightColor = glm::vec3(0.0f, 1.0f, 0.5f);


        glm::vec3 diffuseColor = lightColor * glm::vec3(0.8f); // 降低影响
        glm::vec3 ambientColor = lightColor * glm::vec3(0.1f); // 很低的影响
        lightShader.setVec3("spotlight.ambient", ambientColor);
        lightShader.setVec3("spotlight.diffuse", diffuseColor);
        ...
        lightShader.setVec3("dirlight.direction", glm::vec3(1.0f));
        lightShader.setVec3("dirlight.ambient", ambientColor * 0.5f);
        lightShader.setVec3("dirlight.diffuse", diffuseColor * 0.5f);
        ...
        for( int i = 0; i < 10; i ++ )
        {
            ...
            float angle = 20.0f;
            model = glm::rotate(model, glm::radians(i * angle), glm::vec3(0.5f, 1.0f, 0.0f));
            lightShader.setMat4("model", model);
